...
def student_entrypoint(client_message: ClientMessage):
	...
	# first step is to implement bba-0
	# if below reservoir then choose lowest bitrate
	# if above cushion then choose highest bitrate
	# if between then choose bitrate proportional to how far into the cushion we are
	...
	# else:
	# 	return int((b - r) / c * (client_message.quality_levels - 1))
	...
	if b >= r + c: # above cushion pick highest bitrate
		return client_message.quality_levels - 1
	
	safety_margin = min_chunk_size + fraction * (max_chunk_size - min_chunk_size)
	for i in range(client_message.quality_levels):
		if client_message.quality_bitrates[i] > safety_margin:
